...
public class Discovery extends WatsonService {
  ...
  public ServiceCall<DeleteCollectionResponse> deleteCollection(String environmentId, String collectionId) {
    ...
    RequestBuilder builder = RequestBuilder.delete(String.format("/v1/environments/%s/collections/%s", environmentId,
        collectionId));
    builder.query(VERSION, versionDate);
  ...
  public ServiceCall<Collection> getCollection(String environmentId, String collectionId) {
    Validator.notEmpty(environmentId, "environmentId cannot be null");
    Validator.notEmpty(collectionId, "collectionId cannot be null");
    RequestBuilder builder = RequestBuilder.get(String.format("/v1/environments/%s/collections/%s", environmentId,
        collectionId));
    builder.query(VERSION, versionDate);
    return createServiceCall(builder.build(), ResponseConverterUtils.getObject(Collection.class));
  }
   ...
   * @return the {@link Collection} with the response
   */
  public ServiceCall<Collection> updateCollection(String environmentId, String collectionId, UpdateCollectionRequest
      body) {
    Validator.notEmpty(environmentId, "environmentId cannot be null");
    Validator.notEmpty(collectionId, "collectionId cannot be null");
    RequestBuilder builder = RequestBuilder.put(String.format("/v1/environments/%s/collections/%s", environmentId,
        collectionId));
    builder.query(VERSION, versionDate);
    if (body != null) {
      builder.bodyJson(GsonSingleton.getGson().toJsonTree(body).getAsJsonObject());
   ...
   *
   * @param environmentId the ID of your environment
   * @param configuration Input a JSON object that enables you to customize how your content is ingested and what
   *                      enrichments are added to your data.   `name` is required and must be unique within the
   *                      current `environment`. All other properties are optional.  If the input configuration
   *                      contains the `configuration_id`, `created`, or `updated` properties, then they will be
   *                      ignored and overridden by the system (an error is not returned so that the overridden
   *                      fields do not need to be removed when copying a configuration).   The configuration can
   *                      contain unrecognized JSON fields. Any such fields will be ignored and will not generate an
   *                      error. This makes it easier to use newer configuration files with older versions of the API
   *                      and the service. It also makes it possible for the tooling to add additional metadata and
  ...
  public ServiceCall<Configuration> getConfiguration(String environmentId, String configurationId) {
    Validator.notEmpty(environmentId, "environmentId cannot be null");
    Validator.notEmpty(configurationId, "configurationId cannot be null");
    RequestBuilder builder = RequestBuilder.get(String.format("/v1/environments/%s/configurations/%s", environmentId,
        configurationId));
    builder.query(VERSION, versionDate);
    return createServiceCall(builder.build(), ResponseConverterUtils.getObject(Configuration.class));
  }
   ...
   *                        and what enrichments are added to your data.  The `name` parameter is required and must
   *                        be unique within the current `environment`. All other properties are optional, but if
   *                        they are omitted  the default values replace the current value of each omitted property.
   *                        If the input configuration contains the `configuration_id`, `created`, or `updated`
   *                        properties, they are ignored and overridden by the system, and an error is not returned
   *                        so that the overridden fields do not need to be removed when updating a configuration.
   *                        The configuration can contain unrecognized JSON fields. Any such fields are ignored and
   *                        do not generate an error. This makes it easier to use newer configuration files with
   *                        older versions of the API and the service. It also makes it possible for the tooling to
   *                        add additional metadata and information to the configuration.
   * @return the {@link Configuration} with the response
   */
  public ServiceCall<Configuration> updateConfiguration(String environmentId, String configurationId, Configuration
      configuration) {
    Validator.notEmpty(environmentId, "environmentId cannot be null");
    Validator.notEmpty(configurationId, "configurationId cannot be null");
    Validator.notNull(configuration, "configuration cannot be null");
    RequestBuilder builder = RequestBuilder.put(String.format("/v1/environments/%s/configurations/%s", environmentId,
        configurationId));
    builder.query(VERSION, versionDate);
    builder.bodyJson(GsonSingleton.getGson().toJsonTree(configuration).getAsJsonObject());
   ...
   * @param environmentId   the ID of your environment
   * @param collectionId    the ID of your collection
   * @param configurationId The ID of the configuration to use to process the document. If the `configuration` form
   *                        part is also provided (both are present at the same time), then request will be rejected.
   * @param file            The content of the document to ingest.The maximum supported file size is 50 megabytes.
   *                        Files larger than 50 megabytes is rejected.
   * @param fileMediaType   the media type of file.
   * @param metadata        If you're using the Data Crawler to upload your documents, you can test a document
   *                        against the type of metadata that the Data Crawler might send. The maximum supported
   *                        metadata file size is 1 MB. Metadata parts larger than 1 MB are rejected. Example:  ``` {
   *                        "Creator": "Johnny Appleseed",   "Subject": "Apples" } ```
   * @param configuration   The configuration to use to process the document. If this part is provided, then the
   *                        provided configuration is used to process the document. If the `configuration_id` is also
   *                        provided (both are present at the same time), then request is rejected. The maximum
   *                        supported configuration size is 1 MB. Configuration parts larger than 1 MB are rejected.
   ...
   */
  public ServiceCall<DocumentAccepted> addDocument(String environmentId, String collectionId, String configurationId,
                                                   InputStream file, String fileMediaType, String metadata, String
                                                       configuration) {
    Validator.notEmpty(environmentId, "environmentId cannot be null");
    Validator.notEmpty(collectionId, "collectionId cannot be null");
   ...
   */
  public ServiceCall<DeleteDocumentResponse> deleteDocument(String environmentId, String collectionId, String
      documentId) {
    Validator.notEmpty(environmentId, "environmentId cannot be null");
    Validator.notEmpty(collectionId, "collectionId cannot be null");
   ...
   * @return the {@link DocumentAccepted} with the response
   */
  public ServiceCall<DocumentAccepted> updateDocument(String environmentId, String collectionId, String documentId,
                                                      String configurationId, InputStream file, String fileMediaType,
                                                      String metadata, String configuration) {
    Validator.notEmpty(environmentId, "environmentId cannot be null");
    Validator.notEmpty(collectionId, "collectionId cannot be null");
    Validator.notEmpty(documentId, "documentId cannot be null");
   ...
   * Query documents.
   *
   * See the [Query Language wiki page](https://github.ibm
   * .com/Watson-Discovery/discovery-wiki/blob/master/design/query-language.md) for more details.
   *
   * @param environmentId the ID of your environment
   * @param collectionId  the ID of your collection
   ...
   * generated when ingesting documents. See the [Query Language wiki page](https://github.ibm
   * .com/Watson-Discovery/discovery-wiki/blob/master/design/query-language.md) for more details on the query language.
   *
   * @param environmentId       the ID of your environment
   * @param collectionId        the ID of your collection
   ...
   */
  public ServiceCall<NoticeQueryResponse> queryNotices(String environmentId, String collectionId, QueryNoticesOptions
      queryNoticesOptions) {
    Validator.notEmpty(environmentId, "environmentId cannot be null");
    Validator.notEmpty(collectionId, "collectionId cannot be null");
  ...
  /**
   * Creates a new query template. Query templates can be used to execute complex queries without having to submit
   * the full query on every request. Query templates use the [Mustache](https://mustache.github.io/) or [Handlebars]
   * (http://handlebarsjs.com/) templating syntax. **Template constructs**   * `{{parameter_name}}` - String
   * interpolation. Replace this portion with the specified parameter.   * `{{#if parameter_name}}{{/if}}` - `If`
   * logic helper. Contents of this segment will be included in `parameter_name` is included and valid.   *
   * `{{#default parameter_name Default Value}}` - `Default` logic helper. If `parameter_name` is included, it will
   * be used. Otherwise, the provided default value will be used. For more details, see the [Query Templating design
   * page](https://github.ibm.com/Watson-Discovery/discovery-wiki/blob/master/design/query-templating.md).
   *
   * @param environmentId the ID of your environment
   * @param collectionId  the ID of your collection
   * @param body
   ...
   */
  public ServiceCall<FullQueryTemplate> createQueryTemplate(String environmentId, String collectionId, QueryTemplate
      body) {
    Validator.notEmpty(environmentId, "environmentId cannot be null");
    Validator.notEmpty(collectionId, "collectionId cannot be null");
    Validator.notNull(body, "body cannot be null");
    RequestBuilder builder = RequestBuilder.post(String.format("/v1/environments/%s/collections/%s/templates",
        environmentId, collectionId));
    builder.query(VERSION, versionDate);
    builder.bodyJson(GsonSingleton.getGson().toJsonTree(body).getAsJsonObject());
  ...
  /**
   * Gets a list of existing query templates. For more details, see the [Query Templating design page](https://github
   * .ibm.com/Watson-Discovery/discovery-wiki/blob/master/design/query-templating.md).
   *
   * @param environmentId   the ID of your environment
   * @param collectionId    the ID of your collection
   * @param configurationId The ID of the configuration to use to process the document. If the `configuration` form
   ...
   * @return the {@link DocumentAccepted} with the response
   */
  public ServiceCall<DocumentAccepted> getQueryTemplates(String environmentId, String collectionId, String
      configurationId, InputStream file, String fileMediaType, String metadata, String configuration) {
    Validator.notEmpty(environmentId, "environmentId cannot be null");
    Validator.notEmpty(collectionId, "collectionId cannot be null");
    RequestBuilder builder = RequestBuilder.get(String.format("/v1/environments/%s/collections/%s/templates",
        environmentId, collectionId));
    builder.query(VERSION, versionDate);
    if (configurationId != null) {
      builder.query("configuration_id", RequestUtils.encode(configurationId));
   ...
   *                        supported configuration size is 1 MB. Configuration parts larger than 1 MB are rejected.
   *                        See the `GET /configurations/{configuration_id}` operation for an example configuration.
   * @param step            Specify to only run the input document through the given step instead of running the
   *                        input document through the entire ingestion workflow. Valid values are `convert`,
   ...
   */
  public ServiceCall<TestDocument> testConfigurationInEnvironment(String environmentId, String configuration, String
      step, String configurationId, InputStream file, String fileMediaType, String metadata) {
    Validator.notEmpty(environmentId, "environmentId cannot be null");
