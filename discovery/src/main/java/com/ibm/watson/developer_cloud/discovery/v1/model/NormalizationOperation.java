...
public class NormalizationOperation extends GenericModel {
   ...
   * exists, then the value of the `source_field` overwrites the original value of the `destination_field`.
   * Rename is identical to copy, except that the `source_field` is removed after the value has been copied to the
   ...
   * an array (if it is not an array already). This is ensures the type for `destination_field` is consistent across
   * all documents.
   ...
   * array (if it is not an array already). This is ensures the type for `destination_field` is consistent across
   * all documents.
   * **remove** - Deletes the `source_field` field. The `destination_field` is ignored for this operation.
   * **remove_nulls** - Removes all nested null (blank) leif values from the JSON tree. `source_field` and
   * `destination_field` are ignored by this operation because _remove_nulls_ operates on the entire JSON tree.
