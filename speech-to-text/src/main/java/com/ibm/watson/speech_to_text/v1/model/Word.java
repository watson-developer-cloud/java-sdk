...
public class Word extends GenericModel {
   ...
   * Gets the soundsLike.
   *
   * <p>_For a custom model that is based on a previous-generation model_, an array of as many as
   * five pronunciations for the word. The array can include the sounds-like pronunciation that is
   * automatically generated by the service if none is provided when the word is added to the custom
   * model; the service adds this pronunciation when it finishes processing the word.
   *
   * <p>_For a custom model that is based on a next-generation model_, this field does not apply.
   * Custom models based on next-generation models do not support the `sounds_like` field, which is
   * ignored.
   *
