...
public class SpeechRecognitionResults extends GenericModel {
   ...
   * Gets the warnings.
   *
   * An array of warning messages associated with the request: * Warnings for invalid parameters or fields can include a
   * descriptive message and a list of invalid argument strings, for example, `"Unknown arguments:"` or `"Unknown url
   * query arguments:"` followed by a list of the form `"invalid_arg_1, invalid_arg_2."` * The following warning is
   * returned if the request passes a custom model that is based on an older version of a base model for which an
   * updated version is available: `"Using previous version of base model, because your custom model has been built with
   * it. Please note that this version will be supported only for a limited time. Consider updating your custom model to
   * the new base model. If you do not do that you will be automatically switched to base model when you used the
   * non-updated custom model."` In both cases, the request succeeds despite the warnings.
   *
   * @return the warnings
   */
