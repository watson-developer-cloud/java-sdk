...
/** Unit test class for the SkillImport model. */
public class SkillImportTest {
  final HashMap<String, InputStream> mockStreamMap = TestUtilities.createMockStreamMap();
  final List<FileWithMetadata> mockListFileWithMetadata =
      TestUtilities.creatMockListFileWithMetadata();

  @Test
  public void testSkillImport() throws Throwable {
    ...
    SkillImport skillImportModel =
        new SkillImport.Builder()
            ...
            .type("action")
            .build();
    assertEquals(skillImportModel.name(), "testString");
    assertEquals(skillImportModel.description(), "testString");
    assertEquals(
        skillImportModel.workspace(), java.util.Collections.singletonMap("anyKey", "anyValue"));
    assertEquals(
        skillImportModel.dialogSettings(),
        java.util.Collections.singletonMap("anyKey", "anyValue"));
    assertEquals(skillImportModel.searchSettings(), searchSettingsModel);
    assertEquals(skillImportModel.language(), "testString");
    assertEquals(skillImportModel.type(), "action");

    String json = TestUtilities.serialize(skillImportModel);

    SkillImport skillImportModelNew = TestUtilities.deserialize(json, SkillImport.class);
    assertTrue(skillImportModelNew instanceof SkillImport);
    assertEquals(skillImportModelNew.name(), "testString");
    assertEquals(skillImportModelNew.description(), "testString");
    assertEquals(
        skillImportModelNew.workspace().toString(),
        java.util.Collections.singletonMap("anyKey", "anyValue").toString());
    assertEquals(
        skillImportModelNew.dialogSettings().toString(),
        java.util.Collections.singletonMap("anyKey", "anyValue").toString());
    assertEquals(skillImportModelNew.searchSettings().toString(), searchSettingsModel.toString());
    assertEquals(skillImportModelNew.language(), "testString");
    assertEquals(skillImportModelNew.type(), "action");
  }

  @Test(expectedExceptions = IllegalArgumentException.class)
  public void testSkillImportError() throws Throwable {
    new SkillImport.Builder().build();
  }
}
